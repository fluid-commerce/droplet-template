class CheckoutCallbackController < ApplicationController
  def get_redirect_url
    ...
    user = UPaymentsUserApiClient.new.check_user_exists(email: params[:email], external_id: params[:external_id])

    if user["data"].empty?
      user_payload = UPaymentsConsumerPayloadGenerator.generate_consumer_payload(cart: cart_payload, external_id: params[:external_id])
      user = UPaymentsUserApiClient.new.onboard_consumer(payload: user_payload)
    end

    order_payload = UPaymentsOrderPayloadGenerator.generate_order_payload(cart: cart_payload, external_id: params[:external_id])

    redirect_url = UPaymentsCheckoutApiClient.new.create_order(order_payload:)
  ...
  def get_redirect_url_params
    params.permit(:cart, :external_id, :prefix)
  end
  ...
  def cart_payload
    {
      ...
      available_payment_methods:
        [{id: 11,
          type: "credit_card",
          name: "Credit Card",
          supported_brands: ["visa", "master", "american_express", "discover"],
          test: true,
          sandbox: true,
          bogus_gateway: false,
          tokenize_path: nil,
        ...
        {id: 4, type: "bread", name: "Bread", integration_key: "a21603f7-93b5-4382-ad45-112a77de9391", test: false, sandbox: true}],
