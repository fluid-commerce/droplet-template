class CompanyDropletUninstalledJob < ApplicationJob
  queue_as :default
  ...
  def perform(payload)
    ...
    if company.present?
      ...
      Rails.logger.warn("[CompanyDropletUninstalledJob] Company not found for payload: #{payload.inspect}")
    end
  rescue StandardError => e
    Rails.logger.error("[CompanyDropletUninstalledJob] Error: #{e.class} - #{e.message}")
    raise
  end
